...
// Try to register a service worker if available (improves installability checks).
// This will attempt a HEAD request for /sw.js and register it when present. It's safe — failures are ignored.
(async function registerSWIfAvailable() {
  if ('serviceWorker' in navigator) {
    try {
      // Check if service worker file exists on the server
      ...
      if (res.ok) {
        ...
        console.log('/sw.js not found; skipping service worker registration');
      }
    } catch (err) {
      console.warn('Error while checking /sw.js:', err);
    }
  }
})();
